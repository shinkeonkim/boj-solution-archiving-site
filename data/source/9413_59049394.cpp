...
int main() {
	...
	while(tc--) {
		...
		MCMF mcmf(2 * N + 6, SSRC, SSINK);
		
		mcmf.add_edge(SSRC, SRC, 2, 0);
		mcmf.add_edge(SINK, SSINK, 2, 0);
		
		for1(1, N+1) {
			mcmf.add_edge(SRC, in_node(i), INF, 0);
			mcmf.add_edge(out_node(i), SINK, INF, 0);
		...
		for1(0, M) {
			...
			mcmf.add_edge(out_node(a), in_node(b), 1, 0);
		}
